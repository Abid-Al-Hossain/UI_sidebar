# UI Foundry Sidebar Studio

Standalone visual Sidebar generator built with Next.js, React, and TypeScript.

Standalone Sidebar component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Sidebar UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Sidebar generator
...
The editor is focused on controls that are native to Sidebar and useful in real product work:
- navigation groups, active item, collapse state, badges, profile/footer slot, and rail mode
- width, spacing, density, background, border, shadow, and nested item styling
- desktop/mobile drawer intent and grouped navigation behavior
- semantic nav labels and keyboard-safe links
- React export for sidebar shell

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Sidebar.
...
This repo is intended to publish as Abid-Al-Hossain/UI_sidebar.
